export default function dndScript(shadow) {
  "use strict";
  var currRow = null,
    dragElem = null,
    mouseDownX = 0,
  ...
  const table = shadow.getElementById('table_id');
  const tbody = shadow.getElementById('tbody_id');
  ...
  function bindMouse() {
    ...
    shadow.addEventListener('mouseup', (event) => {
      if (!mouseDrag) return;
  ...
  function addDraggableRow(target) {
    ...
    shadow.dispatchEvent(new MouseEvent('mousemove', {
      view: window,
      cancelable: true,
      bubbles: true
    }));
